...
Azimuth is the dedicated SwiftUI TH-D75 control center and field guide. It mirrors the authenticated 240×180 radio screen, exposes all 25 guarded front-panel keys, reads the complete reviewed 400-record settings catalog, and applies user-approved natural-language proposals through stale-safe verified MCP transactions. Its operational workspaces continuously monitor host-owned APRS/KISS activity and provide explicitly confirmed one-shot message and position transmission. On iPadOS, IF-DSP captures Band B for USB/LSB/CW/AM analysis, spectrum, waterfall, and level diagnostics; bounded retuning uses individually read-back-verified UP/DW steps while unqualified direct FO/FQ writes remain unavailable. Azimuth uses direct USB-C through an embedded DriverKit extension on M-series iPads. On macOS, users can select the public USB serial device or an exact paired Bluetooth address while Azimuth keeps one CAT/MCP owner active at a time. When macOS validates MMDVM on USB-C, the prompt separates a non-destructive same-radio Bluetooth handoff from the explicitly approved operation that turns Menu 650 off and restores USB CAT. iPadOS has no second TH-D75 CAT path while USB-C carries MMDVM, so its prompt explains the required manual Menu 650 recovery instead of offering an operation it cannot perform. Build the core with `./azimuth-core/scripts/build-xcframework.sh`, then generate the app with `(cd azimuth && xcodegen generate)`.
