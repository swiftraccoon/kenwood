...
| [`tmd750-repl/`](tmd750-repl/) | Screen-reader-friendly TM-D750 REPL with USB/native Bluetooth CAT, guarded MCP settings, and automatic Bluetooth D-STAR startup with independent USB restoration | experimental |
...
Azimuth is a native iPadOS and macOS app for the TH-D75: a control center and field guide that puts the radio's own screen and keys on your tablet or Mac and adds the views the radio's small display cannot offer. It has six workspaces:

- **Radio**: the radio's live color display, mirrored on screen, with all 25 front-panel keys as on-screen controls. Every key press is authenticated and followed by a fresh screen capture, so what you see is what the radio shows.
- **APRS**: live packet activity from the radio's built-in TNC (decoded and raw), heard stations on a map, counters, and one-tap message or position transmissions that always ask for confirmation first.
- **IF-DSP**: a spectrum and waterfall view of what Band B is receiving, with USB, LSB, CW, and AM demodulation, signal levels, and loss diagnostics. Retuning happens in small steps that are each read back and verified.
- **Settings**: the complete catalog of the radio's 400 settings with their live values, and the menu numbers the radio uses for them.
- **Assistant**: describe a change in plain language; Azimuth proposes the exact settings edits, writes nothing until you approve, and verifies every write against the radio afterwards.
- **Learn**: a capability center explaining what the radio can do and how Azimuth reaches each feature.

Connections: USB-C on M-series iPads (through an embedded DriverKit extension), USB or a paired Bluetooth radio on a Mac. Azimuth proves it is talking to the intended radio before it will control it, keeps one control link active at a time, and never guesses which attached device is the radio. Reading the full settings snapshot is a separate, explicit step, because the radio restarts when it leaves programming mode; Azimuth reconnects automatically afterwards. When a capture first needs the radio's digital-gateway mode (Menu 650) switched off, Azimuth explains the steps and asks before doing anything; declining leaves the radio untouched, and on iPad, where it cannot perform those steps itself, it explains the manual recovery instead.

For developers: build the Rust core with `./azimuth-core/scripts/build-xcframework.sh`, then generate the Xcode project with `(cd azimuth && xcodegen generate)`. Details live in [`azimuth/README.md`](azimuth/README.md) and [`azimuth-core/README.md`](azimuth-core/README.md).
