...
public actor AzimuthUSBSerialTransport: AzimuthRadioTransport {
    ...
    private func armDoorbell() throws {
        try link.armDoorbell { [weak self] hasData in
            Task { await self?.doorbellFired(hasData: hasData) }
        }
        azimuthUSBTrace("[Azimuth USB] one-shot doorbell armed")
    }

    private func doorbellFired(hasData: Bool) {
        guard !linkDown else { return }
        azimuthUSBTrace("[Azimuth USB] doorbell fired: hasData=\(hasData)")
        guard hasData else {
