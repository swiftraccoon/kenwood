...
pub(crate) fn parse_channel_fields(
    ...
    let rx_frequency = Frequency::from_wire_string(f_rx_freq)?;

    // field 1: TX offset or split TX frequency (10 digits)
    let tx_offset = Frequency::from_wire_string(f_tx_offset)?;
    ...
    let _tx_step = parse_u8_field(f_tx_step, cmd, "tx_step")?;
    let mode_val = parse_u8_field(f_mode, cmd, "mode")?;
    let fine_tuning = parse_u8_field(f_fine_tuning, cmd, "fine_tuning")?;
    let fine_step = parse_u8_field(f_fine_step, cmd, "fine_step")?;
    ...
    let tone_enable = parse_u8_field(f_tone_en, cmd, "tone_enable")? != 0;
    let ctcss_enable = parse_u8_field(f_ctcss_en, cmd, "ctcss_enable")? != 0;
    ...
    let ct_code_val = parse_u8_field(f_ctcss_code, cmd, "ctcss_code")?;
    ...
    let dcs_val = parse_u8_field(f_dcs_code, cmd, "dcs_code")?;
    ...
    let ct_val = parse_u8_field(f_cross_tone_type, cmd, "cross_tone_combo")?;
    ...
    let ds_val = parse_u8_field(f_digital_squelch, cmd, "digital_squelch")?;
    let digital_squelch =
    ...
    let data_mode = parse_u8_field(f_digital_code, cmd, "digital_code")?;
...
fn parse_fq(payload: &str) -> Result<Response, ProtocolError> {
    // Short format: "band,frequency" (exactly one comma, no 21-field tail).
    if let Some((band_str, freq_str)) = payload.split_once(',')
        && !freq_str.contains(',')
    {
        ...
        let rx_frequency = Frequency::from_wire_string(freq_str)?;
