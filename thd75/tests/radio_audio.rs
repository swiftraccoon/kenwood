...
async fn get_dstar_callsign() {
    ...
    let (callsign, suffix) = radio
        .get_dstar_callsign(kenwood_thd75::types::DstarSlot::new(1).unwrap())
        .await
        .unwrap();
...
async fn vox_gain() {
    ...
    let mut radio = Radio::connect(mock).await.unwrap();
    assert_eq!(
        radio.get_vox_gain().await.unwrap(),
        kenwood_thd75::types::VoxGain::new(5).unwrap()
    );
    radio
        .set_vox_gain(kenwood_thd75::types::VoxGain::new(8).unwrap())
        .await
        .unwrap();
}
...
async fn vox_delay() {
    ...
    let mut radio = Radio::connect(mock).await.unwrap();
    assert_eq!(
        radio.get_vox_delay().await.unwrap(),
        kenwood_thd75::types::VoxDelay::new(3).unwrap()
    );
    radio
        .set_vox_delay(kenwood_thd75::types::VoxDelay::new(7).unwrap())
        .await
        .unwrap();
}
