//! Integration tests for qualified high-level memory recall.
...
async fn tune_channel_switches_to_memory_mode() -> TestResult {
    ...
    let mut mock = MockTransport::new();
    // get_regular_channel_record: verify channel is populated
    mock.expect(
        b"ME 021\r",
...
async fn tune_channel_band_b() -> TestResult {
    ...
    let mut radio = Radio::new(mock);
    radio
        .tune_channel(Band::B, RegularChannel::new(42)?)
        .await?;
    Ok(())
}
...
#[tokio::test]
async fn connect_with_tnc_exit_preamble_includes_kiss_exit_frame() -> TestResult {
...
fn mock_modify_page_sequence(
    ...
    let write_cmd = programming::build_write_command(WritableMcpPage::new(page)?, expected);
...
async fn modify_memory_page_applies_closure() -> TestResult {
    ...
    let mut radio = Radio::new(mock);
    radio
        .modify_memory_page(WritableMcpPage::new(page)?, |data| {
...
async fn modify_memory_page_rejects_factory_cal_page() -> TestResult {
    ...
    // while constructing the typed write address, before MCP I/O is possible.
